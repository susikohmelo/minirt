...
/*   Updated: 2025/02/19 17:27:16 by lfiestas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_click	click_type(t_minirt *m)
{
	...
	click_time[0] = mlx_get_time();
	click_time[1] = last_click_time;
	last_click_time = click_time[0];
	if (m->shape_type != SHAPE_NO_SHAPE
		&& clicked_menu && 2 * CHAR_HEIGHT <= m->mouse_y)
		return (CLICK_SLIDER);
	if (click_time[0] - click_time[1] < .2
